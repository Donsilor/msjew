...
class MenuController extends BaseController
{
    ...
    public function actionFront()
    {
        ...
        $query = $this->modelClass::find()
            ->orderBy('sort asc, id asc')
            ->filterWhere(['cate_id' => $cate_id])
            ->andWhere(['app_id' => AppEnum::FRONTEND]);
        $dataProvider = new ActiveDataProvider([
            'query' => $query,
            'pagination' => false
        ]);
        ...
        return $this->render('front', [
            'dataProvider' => $dataProvider,
            'cates' => Yii::$app->services->menuCate->findDefault(AppEnum::FRONTEND),
            'cate_id' => $cate_id,
        ]);
    }
    ...
    public function actionFrontEditLang()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if($model->save()){
                $this->editLang($model);
                $this->redirect(['front', 'cate_id' => $model->cate_id]);
            }else{
                $this->message($this->getError($model), $this->redirect(['front', 'cate_id' => $model->cate_id]), 'error');
            }
        ...
        return $this->renderAjax('ajax-edit-lang', [
            'model' => $model,
            'cates' => Yii::$app->services->menuCate->getDefaultMap(AppEnum::FRONTEND),
            'menuDropDownList' => Yii::$app->services->menu->getDropDown($menuCate, AppEnum::FRONTEND, $id),
        ]);
    }
